...
@Service // A classe é um componente de serviço gerenciado pelo Spring para operações com a entidade User
public class UserService {
	...
	public void delete(Long id) {
		try {
			// Verifica a existência de um determinado id no banco de dados, caso exista, é
			// feita a deleção
			...
			/*
			 * Captura exceções relacionadas a violação de integridade do banco de dados,
			 * como deletar um usuário referenciado em uma chave estrangeira, e lança um
			 * DatabaseExcpetion
			 */
	...
	public User update(Long id, User obj) {
		try {
			...
			throw new ResourceNotFoundException(id); // Lança uma exceção caso o objeto User em questão não seja
														// encontrado
